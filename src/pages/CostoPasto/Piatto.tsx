...
const Piatto = ({ elementiRistorazione, portata }: any) => {
  let totIngrArray: number[] = []

  const multiply = (qty: number, costoUm: number) => qty * costoUm;
  const sum = (array: number[]) => array.reduce((acc, num) => acc + num, 0).toFixed(1);

  const piatti =  Object.keys(elementiRistorazione[portata]).map(piatto => {
    totIngrArray = []
    return(
      <Fragment>
        {
